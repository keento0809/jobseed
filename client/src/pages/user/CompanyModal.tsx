...
const CompanyModal = ({ showModal, setShowModal }: modalProps) => {
  ...
  const { ref, isComponentVisible, setIsComponentVisible } =
    useDetectClickOutside({ initialVisible: false });
    ...
    <div className="bg-modal relative z-[1001]">
      <div className="modal-container wrapper py-6">
        ...
        <div className="text-sm relative">
          ...
          >
            {companyData.company_size.length === 0
              ? "0-"
              : companyData.company_size}
          </div>
          {isComponentVisible && (
            <CompanySizeDropDown onClick={handleSetCompany} />
          )}
        </div>
        <Text_field_lg name={"description"} onChange={companyDataHandler} />
